...
import { getSavedResumes, getTemplateDetailEditByUser } from "../../api";
import { fadeInOut, opacityINOut } from "../../animation";
...
const Template1 = () => {
  ...
  const templateName =  pathname?.split("/")?.slice(-2, -1)[0];
//   console.log(templateName)
  ...
  const {
    ...
    //  queryKey:["templateEditedByUser", `${templateName}-111111`],
     queryFn:() => getTemplateDetailEditByUser(user?.uid, `${templateName}-${user?.uid}`)
  }
  );
...
//   const {    resumeData } =   '';
//   const {    resume_isLoading } =   true;
//   const {    resume_isError } =   false;
//   const {    refetch_resumeData } =   ()=>console.log("Refetch is Invoked");
  ...
  const generatePDF = async () => {
    ...
    const element=resumeRef.current;
    if(!element){
        toast.info("Unable to Capture at this moment");
        return;
    }

    htmlToImage.toPng(element).then(
        (dataUrl)=>{
            const a4Width=210;
            const a4Height=297;
  ...
  const generateSvg = async () => {
    ...
    htmlToImage.toSvg(element).then(
        (dataUrl)=>{
            const a=document.createElement("a");
            a.href=dataUrl;
  ...
  if (resume_isError) {
    console.log('Error:',resume_error) 
  }


  return (
    <div className="w-full flex flex-col items-center justify-start gap-4 py-12">
      {/* bread crump */}
      ...
      <div className="w-full lg:w-[1200px] grid grid-cols-1 lg:grid-cols-12 px-6 lg:px-32">
        ...
        <div className="col-span-12 px-4 py-6">
          ...
          <div className="w-full h-auto grid grid-cols-12 relative overflow-hidden" ref={resumeRef}>
            ...
            <AnimatePresence>
            {isSaving&&<motion.div {...fadeInOut} className="absolute left-0 top-0 h-full w-full inset-1 bg-[rgba(0,0,0,0.4)] z-50 flex flex-col gap-4 items-center justify-center" >
                    <PuffLoader color='#fff' size={80}/>  
                    <p className="text-white text-sm tracking-wide">Saving...</p>
            </motion.div>}
            </AnimatePresence>
          </div>
        </div>
